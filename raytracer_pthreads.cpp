...
// [/header]
// [compile]
// g++ -o raytracer_pthreads -Wall raytracer_pthreads.cpp -lrt -pthread
// [/compile]
// [ignore]
...
//[/comment]
void *render_pthread(void *threadid)
{
    ...
    for (unsigned y = 0; y < height; y++) {
        for (unsigned x = ((t_id)*(width/NUM_THREADS)); x < ((t_id+1)*(width/NUM_THREADS)); x++) 
        {
    ...
    //printf("REACHED\n");
    // int rc = pthread_barrier_wait(&theBarrier);
...
int main(int argc, char **argv)
{
    ...
    // if(pthread_barrier_init(&theBarrier, NULL, NUM_THREADS))
    // {
    //     printf("Barrier aborted\n");
    // }
    ...
    for(int i = 0; i < NUM_THREADS; i++){
        ...
        rc = pthread_create(&threads[i], NULL, render_pthread, &thread_data_array[i]);
        if (rc) {
    ...
    {
        if(pthread_join(threads[x],NULL))
        {
            printf("Rouge threads ahead!\n");
        }
    }
    // Save result to a PPM image (keep these flags if you compile under Windows)
    std::ofstream ofs("./pthreads.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
